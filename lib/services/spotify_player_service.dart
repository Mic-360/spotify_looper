/// Spotify Web Playback SDK service for browser-based playback.
library;
...
class SpotifyPlayerService {
  ...
  Future<void> initialize(String accessToken) async {
    if (!kIsWeb) {
      debugPrint(
        'SpotifyPlayerService: Web Playback SDK is only available on web',
      );
      return;
    }

    debugPrint('SpotifyPlayerService: Initializing with token');

    try {
      final spotifyLooper = globalContext['spotifyLooper'] as JSObject?;
      if (spotifyLooper == null) {
        debugPrint('SpotifyPlayerService: window.spotifyLooper not found');
        return;
      }
      ...
      if (globalContext['Spotify'] != null) {
        debugPrint(
          'SpotifyPlayerService: Spotify SDK found, calling _initPlayer',
        );
        ...
        debugPrint(
          'SpotifyPlayerService: Waiting for Spotify SDK to load (onSpotifyWebPlaybackSDKReady will handle it)',
        );
      }
    } catch (e) {
      debugPrint('SpotifyPlayerService: Error initializing: $e');
    }
  }
  ...
  void updateToken(String token) {
    ...
    try {
      ...
      if (spotifyLooper != null) {
        ...
        debugPrint('SpotifyPlayerService: Token updated');
      }
    } catch (e) {
      debugPrint('SpotifyPlayerService: Error updating token: $e');
    }
  }
  ...
  void _onPlayerReady(String deviceId) {
    ...
    debugPrint('SpotifyPlayerService: Player ready with device ID: $deviceId');
  }
  ...
  void _onPlayerStateChanged(JSObject? state) {
    ...
    try {
      ...
      _stateController.add(_currentState);
      debugPrint(
        'SpotifyPlayerService: State updated: ${!isPaused ? 'Playing' : 'Paused'} - $trackName',
      );
    } catch (e) {
      debugPrint('SpotifyPlayerService: Error parsing state: $e');
    }
  }

  void _onPlayerError(String message) {
    debugPrint('SpotifyPlayerService: SDK Error: $message');
  ...
  Future<void> play(String uri) async {
    if (!_isReady || _deviceId == null) {
      debugPrint('SpotifyPlayerService: Player not ready');
      return;
    }

    debugPrint('SpotifyPlayerService: Playing $uri on device $_deviceId');
